...
#On peut creer une fonction à l'interieur de l'appel à lapply.
#Le premier paramètre est necessairement un element du vecteur
...
#dans la mesure où ces derniers sont des listes. Cela s'avere
#pratique pour réaliser des operations pour chaque colonne 
#d'un tableau de donnees. Afin de prendre moins de place 
#dans l'affichage, l'exemple suivant utilise la fonction
...
#La fonction sapply applique une fonction aux elements d'un vecteur 
# ou d'une liste et peut retourner un vecteur, une liste ou une 
# matrice. Elle possede la syntaxe suivante :
...
#La fonction vapply
#La fonction vapply est similaire à sapply, mais elle
#possede un type de valeurs specifie, ce qui peut rendre
#l'utilisation plus sure (et parfois plus rapide). 
#Lorsqu'on lui fournit un data.frame, vapply retourne le 
#meme resultat que sapply. Cependant, quand on lui fournit 
#une liste vide, vapply retourne un vecteur logique de longueur 
#nulle (ce qui est plus sense que la liste vide que returne
#sapply).
...
#La fonction apply
#La fonction apply possede la syntaxe suivante :
...
#La fonction tapply s'applique a chaque cellule d'un tableau,
...
#scalaire en une fonction vectorielle. 
#Attention, cela ne permet pas d'ameliorer la rapidite d'exécution 
#du code.

#Par contre, son utilisation assez intuitive permet de 
#gagner du temps. Il s'agit donc de faire l'arbitrage 
#entre le temps passe a trouver un moyen elegant et 
#efficace pour effectuer une operation en passant par de 
#reels calculs vectoriels et le gain d'execution que ce
#calcul vectoriel apporte vis-a-vis d'une boucle. 
