...
#En R, un facteur (ou factor) est un vecteur contenant uniquement 
# certaines valeurs predefinies.Les valeurs pre-definies sont
# appelees des levels

c("orange", "banane", "fraise")

# On tire aleatoirement dans ce vecteur grace a la fonction sample() 
# avec l'option size=10 et replace = TRUE (tirage avec remise)
...
# Comme il prend un nombre fini de valeurs, on pourrait aussi le definir 
# comme un facteur :
x <- factor(x = x)
x
class(x)
...
#Variables qualitatives ou categorielles expriment une qualite comme
# le sexe, le metier ou le nom
